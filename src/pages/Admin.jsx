...
export default function Admin({ onBack }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f5f5f5' }}>
      {/* Sidebar */}
      <div style={{ width: sidebarOpen ? 220 : 60, background: '#161616', color: '#fff', flexShrink: 0, transition: 'width 0.3s ease', overflow: 'hidden', position: 'sticky', top: 0, height: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, padding: '8px 6px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {SIDEBAR_ITEMS.map(item => (
            <button key={item.id} onClick={() => setTab(item.id)} style={{
              ...
              color: tab === item.id ? '#e93d3d' : '#999',
              fontSize: 13, fontWeight: tab === item.id ? 700 : 400, whiteSpace: 'nowrap',
              transition: 'all 0.2s', borderLeft: tab === item.id ? '3px solid #e93d3d' : '3px solid transparent',
            }}>
              ...
              {sidebarOpen && item.badge && activeOrders.length > 0 && (
                <span style={{ marginLeft: 'auto', background: '#e93d3d', color: '#fff', fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 100 }}>{activeOrders.length}</span>
              )}
            </button>
          ))}
        </div>
        <button onClick={onBack} style={{ padding: '14px 12px', borderTop: '1px solid #333', fontSize: 12, color: '#666', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ flex: 1, padding: 24, overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 0, border: B, marginBottom: 24, background: '#fff' }}>
          {[
            ...
            { label: 'CATERING', value: catering.filter(c => c.status === 'New').length, sub: 'pending', color: '#e93d3d' },
          ].map((k, i) => (
    ...
    <div>
      ...
      {activeOrders.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontFamily: V.font_display, fontSize: 18, marginBottom: 8, color: '#e93d3d' }}>🔴 ACTIVE ORDERS</div>
          ...
          {topItems.map((item, i) => (
            <div key={item.name} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 16px', borderBottom: i < topItems.length - 1 ? B : 'none', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontFamily: V.font_display, fontSize: 16, color: i === 0 ? '#e93d3d' : '#ccc', minWidth: 28 }}>#{i + 1}</span>
        ...
        <div style={{ border: B, background: '#fff' }}>
          ...
          {customers.sort((a, b) => b.totalSpent - a.totalSpent).slice(0, 5).map((c, i) => (
            <div key={c.phone} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 16px', borderBottom: i < 4 ? B : 'none', alignItems: 'center' }}>
              ...
              <span style={{ fontFamily: V.font_display, fontSize: 18, color: '#e93d3d' }}>${c.totalSpent}</span>
            </div>
          ))}
        </div>
      </div>
...
function OrderCard({ order: o, onStatus, borderRight }) {
  const statusColors = { New: '#e93d3d', Preparing: '#F5A623', Ready: '#37ca37' }
  ...
  return (
    <div style={{ padding: 16, borderRight: borderRight ? B : 'none', background: '#fff' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
        <span style={{ fontFamily: V.font_display, fontSize: 22, color: '#e93d3d' }}>${o.total?.toFixed(2)}</span>
        {next[o.status] && <button onClick={() => onStatus(o.id, next[o.status])} style={{ padding: '6px 14px', background: '#e93d3d', color: '#fff', fontFamily: V.font_display, fontSize: 12, border: 'none', cursor: 'pointer' }}>→ {next[o.status].toUpperCase()}</button>}
      </div>
    </div>
  )
}
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 0, border: B, marginBottom: 16 }}>
        {['All', 'New', 'Preparing', 'Ready', 'Picked Up', 'Delivered'].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{ padding: '10px 16px', fontFamily: V.font_display, fontSize: 14, background: filter === f ? '#e93d3d' : '#fff', color: filter === f ? '#fff' : '#000', borderRight: B, flex: 1 }}>{f.toUpperCase()}</button>
        ))}
      </div>
    ...
    <div>
      ...
      {MENU.map(cat => (
        <div key={cat.category} style={{ marginBottom: 20 }}>
          <div style={{ fontFamily: V.font_display, fontSize: 20, color: '#e93d3d', marginBottom: 8, borderBottom: B, paddingBottom: 6 }}>{cat.category.toUpperCase()}</div>
          <div style={{ border: B, background: '#fff' }}>
...
function CustomersTab({ customers }) {
  return (
    <div>
      ...
      <div style={{ border: B, background: '#fff' }}>
        <div style={{ display: 'flex', padding: '10px 16px', borderBottom: B, background: '#f5f5f5' }}>
        ...
        {customers.sort((a, b) => b.totalSpent - a.totalSpent).map((c, i) => (
          <div key={c.phone} style={{ display: 'flex', padding: '12px 16px', borderBottom: i < customers.length - 1 ? B : 'none', alignItems: 'center' }}>
            ...
            <span style={{ flex: 1, textAlign: 'center', fontFamily: V.font_display, fontSize: 16, color: '#e93d3d' }}>${c.totalSpent}</span>
      ...
      <div style={{ display: 'flex', gap: 0, border: B, background: '#fff', marginBottom: 24 }}>
        {[
          ...
          <div key={k.label} style={{ flex: 1, padding: 16, borderRight: i < 3 ? B : 'none', textAlign: 'center' }}>
            <div style={{ fontFamily: V.font_display, fontSize: 28, color: '#e93d3d' }}>{k.value}</div>
      ...
      <div style={{ border: B, background: '#fff' }}>
        {[
          ...
          <div key={r.pts} style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', borderBottom: i < 3 ? B : 'none' }}>
            <span style={{ fontFamily: V.font_display, fontSize: 20, color: '#e93d3d', minWidth: 80 }}>{r.pts} PTS</span>
        ...
        {requests.map((r, i) => (
          <div key={r.id} style={{ padding: 16, borderBottom: i < requests.length - 1 ? B : 'none' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', background: r.status === 'Confirmed' ? '#37ca37' : '#e93d3d', color: '#fff' }}>{r.status.toUpperCase()}</span>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 0, border: B, background: '#fff' }}>
        {LOCATIONS.map((l, i) => (
          <div key={l.id} style={{ padding: 16, borderRight: i % 2 === 0 ? B : 'none', borderBottom: i < LOCATIONS.length - 2 ? B : 'none' }}>
            ...
            <div style={{ fontSize: 13, color: '#e93d3d', fontWeight: 600 }}>{l.phone}</div>
            {l.flag && <div style={{ marginTop: 6, display: 'inline-block', padding: '2px 8px', background: '#e93d3d', color: '#fff', fontSize: 10, fontWeight: 700 }}>{l.flag}</div>}
          </div>
        ))}
      </div>
    </div>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 120 }}>
            {dayRevs.map((rev, i) => (
              <div key={i} style={{ flex: 1, textAlign: 'center' }}>
                ...
                <div style={{ height: `${Math.max((rev / maxRev) * 80, 4)}px`, background: '#e93d3d', opacity: 0.4 + (rev / maxRev) * 0.6 }} />
        ...
        <div style={{ border: B, background: '#fff', padding: 20 }}>
          ...
          {topItems.map((item, i) => (
            <div key={item.name} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0' }}>
              <span style={{ fontSize: 13 }}><span style={{ fontFamily: V.font_display, color: i === 0 ? '#e93d3d' : '#ccc', marginRight: 6 }}>#{i + 1}</span>{item.name}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 0, border: B }}>
          {[
            ...
            <div key={k.label} style={{ padding: 14, borderRight: i < 3 ? B : 'none', textAlign: 'center' }}>
              <div style={{ fontFamily: V.font_display, fontSize: 24, color: '#e93d3d' }}>{k.val}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 0, border: B, background: '#fff' }}>
        {[
          ...
          <div key={m.title} style={{ padding: 20, borderRight: i % 2 === 0 ? B : 'none', borderBottom: i < 2 ? B : 'none', textAlign: 'center' }}>
            ...
            <button style={{ padding: '8px 20px', background: '#e93d3d', color: '#fff', fontFamily: V.font_display, fontSize: 14, border: 'none', cursor: 'pointer' }}>{m.btn}</button>
          </div>
        ))}
      </div>
    </div>
