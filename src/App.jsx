...
export default function App() {
  ...
  return (
    <>
      ...
      <div style={{ ...drw.panel, transform: cartOpen ? 'translateX(0)' : 'translateX(100%)' }}>
        ...
        <div style={drw.toggleRow}>
          {['pickup', 'delivery'].map(t => (
            <button key={t} onClick={() => setOrderType(t)} style={{ ...drw.toggleBtn, background: orderType === t ? '#D42B20' : '#1e1e1e', color: orderType === t ? '#fff' : '#8a8078' }}>{t === 'pickup' ? '🏃 Pickup' : '🚗 Delivery'}</button>
          ))}
        </div>
        ...
        <div style={drw.items}>
          {cart.length === 0 ? <div style={{ padding: '48px 20px', textAlign: 'center', color: '#5a5450' }}>Your cart is empty</div> : cart.map(item => (
            <div key={item.id} style={drw.item}>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, color: '#F5F0E8', fontSize: 14 }}>{item.name}</div>
                <div style={{ fontSize: 12, color: '#5a5450' }}>${item.price} each</div>
              </div>
              ...
              <div style={{ minWidth: 50, textAlign: 'right', fontWeight: 700, color: '#D42B20', fontSize: 14 }}>${(item.price * item.qty).toFixed(2)}</div>
            </div>
          ))}
        </div>
        {cart.length > 0 && (
          <div style={drw.footer}>
            ...
            <button onClick={clearCart} style={{ width: '100%', padding: '10px 0', color: '#5a5450', fontSize: 12, marginTop: 8 }}>Clear Cart</button>
          </div>
        )}
      </div>
      ...
      <section style={hero.wrap}>
        ...
        <div style={hero.content}>
          ...
          <h1 style={{ opacity: heroVis ? 1 : 0, transform: heroVis ? 'none' : 'translateY(24px)', transition: 'all 0.8s ease 0.5s' }}>
            <span style={hero.h1}>TACO</span><br /><span style={{ ...hero.h1, color: '#D42B20' }}>BOY'S</span>
          </h1>
      ...
      {/* ═══ ABOUT ═══ */}
      <section style={{ padding: '80px 24px', background: '#0e0e0e' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 40, flexWrap: 'wrap', marginBottom: 32 }}>
              {[{ val: '6', l: 'LOCATIONS' }, { val: '94K', l: 'FOLLOWERS' }, { val: '2019', l: 'FOUNDED' }, { val: '1K+', l: 'YELP REVIEWS' }].map(s2 => (
                <div key={s2.l} style={{ textAlign: 'center' }}>
                  <div style={{ fontFamily: "'Bebas Neue'", fontSize: 36, color: '#F5F0E8', letterSpacing: '0.05em' }}>{s2.val}</div>
                  <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, letterSpacing: '0.2em', color: '#5a5450' }}>{s2.l}</div>
                </div>
              ))}
            </div>
          </Reveal>
          <Reveal delay={200}>
            <p style={{ fontSize: 16, lineHeight: 1.8, color: '#8a8078', maxWidth: 600, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14, marginTop: 24 }}>
            {SPECIALS.map((sp, i) => (
              <Reveal key={sp.name} delay={100 + i * 100}>
                <div style={{ background: '#151515', border: '1px solid #2a2a2a', borderRadius: 12, padding: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    <span style={{ fontFamily: "'Bebas Neue'", fontSize: 20, color: '#F5F0E8', letterSpacing: '0.05em' }}>{sp.name}</span>
                  ...
                  <div style={{ fontSize: 13, color: '#8a8078', lineHeight: 1.5, marginBottom: 6 }}>{sp.desc}</div>
                  <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: '#5a5450' }}>{sp.day}</div>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* ═══ MENU ═══ */}
      <section id="menu" style={{ padding: '100px 24px', background: '#0e0e0e' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <Reveal><div style={tag}>THE MENU</div><h2 style={h2}>Mesquite Charcoal. Fresh Tortillas.<br /><span style={{ color: '#D42B20' }}>Real Food.</span></h2></Reveal>
          <Reveal delay={100}>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 32 }}>
              {MENU.map(c => (
                <button key={c.category} onClick={() => setActiveCategory(c.category)} style={{ padding: '8px 18px', borderRadius: 100, fontSize: 12, fontWeight: 600, letterSpacing: '0.05em', border: '1px solid', transition: 'all 0.3s ease', fontFamily: "'DM Sans'", background: activeCategory === c.category ? '#D42B20' : '#151515', color: activeCategory === c.category ? '#fff' : '#8a8078', borderColor: activeCategory === c.category ? '#D42B20' : '#2a2a2a' }}>
                  {c.category}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 12 }}>
            {MENU.find(c => c.category === activeCategory)?.items.map((item, i) => (
              <Reveal key={item.id} delay={i * 60}>
                <div style={{ background: '#151515', border: `1px solid ${addedFlash === item.id ? '#D42B20' : '#2a2a2a'}`, borderRadius: 12, padding: 18, transition: 'border-color 0.3s ease' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                    <div>
                      <div style={{ fontSize: 16, fontWeight: 700, color: '#F5F0E8', marginBottom: 4 }}>{item.name}{item.hot && <span style={{ marginLeft: 6, fontSize: 12 }}>🔥</span>}</div>
                      <div style={{ fontSize: 13, color: '#8a8078', lineHeight: 1.5 }}>{item.desc}</div>
                    </div>
                    <div style={{ fontFamily: "'Bebas Neue'", fontSize: 22, color: '#D42B20', letterSpacing: '0.05em', flexShrink: 0 }}>${item.price}</div>
                  </div>
                  <button onClick={() => doAddToCart(item)} style={{ width: '100%', padding: '10px 0', borderRadius: 100, border: `1px solid ${addedFlash === item.id ? '#D42B20' : '#2a2a2a'}`, fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.3s ease', fontFamily: "'DM Sans'", background: addedFlash === item.id ? 'rgba(212,43,32,0.1)' : 'transparent', color: addedFlash === item.id ? '#D42B20' : '#8a8078' }}>
          ...
          <Reveal delay={200}>
            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', background: '#151515', border: '1px solid #2a2a2a', borderRadius: 12, padding: 20, marginTop: 32, maxWidth: 600, marginLeft: 'auto', marginRight: 'auto' }}>
              <div style={{ fontSize: 28, flexShrink: 0 }}>🌶️</div>
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#F5F0E8', marginBottom: 4 }}>The Salsa Bar</div>
                <div style={{ fontSize: 14, color: '#8a8078', lineHeight: 1.6 }}>Every order comes with our legendary self-serve salsa bar — creamy avocado, green tomatillo, fiery red habanero, plus radishes, cucumbers, pico de gallo, cilantro, onion, cabbage, and lime.</div>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <section style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <Reveal delay={100}>
            <p style={{ fontSize: 15, color: '#8a8078', textAlign: 'center', maxWidth: 500, margin: '0 auto 32px', lineHeight: 1.6 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12 }}>
            {MILL_AVE_COCKTAILS.map((c, i) => (
              <Reveal key={c.name} delay={150 + i * 80}>
                <div style={{ background: '#151515', border: '1px solid #2a2a2a', borderRadius: 12, padding: 18 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: '#F5F0E8' }}>{c.name}</span>
                    <span style={{ fontFamily: "'Bebas Neue'", fontSize: 18, color: '#F5A623' }}>${c.price}</span>
                  </div>
                  <div style={{ fontSize: 12, color: '#8a8078', lineHeight: 1.5 }}>{c.desc}</div>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* ═══ CATERING ═══ */}
      <section id="catering" style={{ padding: '100px 24px', background: '#0e0e0e' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          <Reveal><div style={tag}>CATERING</div><h2 style={h2}>Feed Your <span style={{ color: '#D42B20' }}>Whole Crew.</span></h2>
          <p style={{ fontSize: 15, color: '#8a8078', lineHeight: 1.6, maxWidth: 460, margin: '0 auto 40px' }}>Corporate events, birthday parties, game day — we cater everything. Mesquite-grilled meats, fresh tortillas, and the full salsa bar.</p></Reveal>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            {[
              ...
              <Reveal key={pkg.name} delay={100 + i * 100}>
                <div style={{ background: '#151515', border: '1px solid #2a2a2a', borderRadius: 16, padding: 28, textAlign: 'center' }}>
                  <div style={{ fontFamily: "'Bebas Neue'", fontSize: 36, color: '#D42B20', letterSpacing: '0.05em', marginBottom: 4 }}>{pkg.price}</div>
                  <div style={{ fontFamily: "'Bebas Neue'", fontSize: 22, color: '#F5F0E8', letterSpacing: '0.05em', marginBottom: 4 }}>{pkg.name}</div>
                  <div style={{ fontSize: 12, color: '#F5A623', fontWeight: 600, marginBottom: 12 }}>Serves {pkg.serves}</div>
                  <div style={{ fontSize: 13, color: '#8a8078', lineHeight: 1.6, marginBottom: 20 }}>{pkg.desc}</div>
                  <button onClick={() => setCateringOpen(true)} style={{ padding: '12px 28px', borderRadius: 100, background: 'transparent', border: '1px solid #2a2a2a', color: '#F5F0E8', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Book Catering</button>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
          ...
          <Reveal delay={100}>
            <div style={{ background: 'linear-gradient(135deg, #D42B20, #F5A623)', borderRadius: 16, padding: 3, marginTop: 32 }}>
              <div style={{ background: '#151515', borderRadius: 14, padding: 28 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <div><div style={{ fontFamily: "'Bebas Neue'", fontSize: 28, letterSpacing: '0.05em' }}>TACO BOY'S REWARDS</div><div style={{ fontSize: 12, color: '#8a8078' }}>Earn 1 point per dollar spent</div></div>
                  <div style={{ fontSize: 36 }}>🌮</div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                  {[{ pts: '50 pts', reward: 'Free Drink' }, { pts: '100 pts', reward: 'Free Taco' }, { pts: '250 pts', reward: 'Free Plato' }].map(r => (
                    <div key={r.pts} style={{ background: '#1e1e1e', borderRadius: 10, padding: 14 }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: '#F5A623', fontFamily: "'Bebas Neue'", letterSpacing: '0.05em' }}>{r.pts}</div>
                      <div style={{ fontSize: 12, color: '#8a8078' }}>{r.reward}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* ═══ LOCATIONS ═══ */}
      <section id="locations" style={{ padding: '100px 24px', background: '#0e0e0e' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}>
          <Reveal><div style={tag}>LOCATIONS</div><h2 style={h2}>6 Spots Across <span style={{ color: '#D42B20' }}>The Valley.</span></h2></Reveal>
            ...
            {LOCATIONS.map((l, i) => (
              <Reveal key={l.id} delay={100 + i * 80}>
                <div style={{ background: '#151515', border: '1px solid #2a2a2a', borderRadius: 14, padding: 22, textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: 13, color: '#8a8078', marginBottom: 6 }}>{l.addr}</div>
      ...
      <section style={{ padding: '60px 24px' }}>
        <div style={{ maxWidth: 500, margin: '0 auto', textAlign: 'center' }}>
          <Reveal>
            ...
            <div style={{ fontSize: 14, color: '#8a8078', marginBottom: 20 }}>New menu items, promos, and events — straight to your inbox.</div>
            <form onSubmit={e => { e.preventDefault(); alert('Subscribed! 🌮') }} style={{ display: 'flex', gap: 8 }}>
              <input type="email" placeholder="your@email.com" required style={{ flex: 1, padding: '12px 16px', borderRadius: 100, border: '1px solid #2a2a2a', background: '#151515', color: '#F5F0E8', fontSize: 14, fontFamily: "'DM Sans'" }} />
      ...
      {/* ═══ CTA ═══ */}
      <section style={{ padding: '120px 24px', background: 'linear-gradient(180deg, #0a0a0a 0%, #1a0a08 50%, #0a0a0a 100%)', textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          <Reveal>
            <div style={{ fontSize: 48, marginBottom: 20 }}>🔥</div>
            <h2 style={{ fontFamily: "'Bebas Neue'", fontSize: 'clamp(40px, 8vw, 80px)', color: '#F5F0E8', letterSpacing: '0.03em', lineHeight: 1 }}>STOP SCROLLING.<br /><span style={{ color: '#D42B20' }}>START EATING.</span></h2>
          </Reveal>
          <Reveal delay={100}>
      ...
      <footer style={{ padding: '48px 24px 32px', textAlign: 'center', borderTop: '1px solid #2a2a2a' }}>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 16, fontSize: 13, color: '#8a8078' }}>
          ...
          <a href="#admin" style={{ color: '#5a5450' }}>Admin</a>
        </div>
        <div style={{ fontSize: 11, color: '#5a5450', opacity: 0.5 }}>© 2026 Taco Boy's. All rights reserved. Phoenix, AZ</div>
      </footer>
    </>
  )
}
  ...
  return (
    <div style={mod.backdrop} onClick={onClose}><div style={mod.box} onClick={e => e.stopPropagation()}>
      <div style={mod.header}>
        ...
        <button onClick={onClose} style={{ fontSize: 20, color: '#8a8078' }}>✕</button>
      </div>
      {step === 1 ? (
        <div style={mod.body}>
        ...
        <div style={mod.body}>
          <div style={{ fontSize: 12, color: '#8a8078', marginBottom: 8 }}>{orderType === 'pickup' ? `Pickup at ${location.name}` : `Delivery to ${form.address}`}{form.time && ` · ${form.time}`}</div>
          {cart.map(item => (
            <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #2a2a2a', fontSize: 14 }}>
              <span>{item.name} ×{item.qty}</span><span style={{ color: '#D42B20', fontWeight: 600 }}>${(item.price * item.qty).toFixed(2)}</span>
            </div>
          ))}
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 0 8px', fontWeight: 700, fontSize: 16 }}>
            <span>Total</span><span style={{ color: '#D42B20', fontFamily: "'Bebas Neue'", fontSize: 24 }}>${total.toFixed(2)}</span>
          </div>
          <button onClick={() => onComplete(form)} style={{ ...hero.btnPrimary, width: '100%', textAlign: 'center', marginTop: 8 }}>Place Order</button>
          <button onClick={() => setStep(1)} style={{ width: '100%', padding: '10px 0', color: '#8a8078', fontSize: 12, marginTop: 8 }}>← Back</button>
        </div>
      )}
    </div></div>
  )
  ...
  return (
    <div style={mod.backdrop} onClick={onClose}><div style={mod.box} onClick={e => e.stopPropagation()}>
      <div style={{ textAlign: 'center', padding: '40px 24px' }}>
        ...
        <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 14, color: '#D42B20', marginBottom: 12 }}>{order.id}</div>
        <div style={{ color: '#8a8078', fontSize: 14, lineHeight: 1.6, marginBottom: 4 }}>{order.type === 'pickup' ? `Pick up at ${order.location}` : 'Delivery on its way'}</div>
        <div style={{ color: '#5a5450', fontSize: 13 }}>We'll text you when it's ready.</div>
        <div style={{ fontFamily: "'Bebas Neue'", fontSize: 28, color: '#D42B20', margin: '16px 0' }}>${order.total?.toFixed(2)}</div>
...
function CateringModal({ onClose }) {
  ...
  if (done) return (
    <div style={mod.backdrop} onClick={onClose}><div style={mod.box} onClick={e => e.stopPropagation()}>
      <div style={{ textAlign: 'center', padding: '40px 24px' }}>
        ...
        <div style={{ color: '#8a8078', fontSize: 14, lineHeight: 1.6 }}>We'll get back to you within 24 hours.</div>
  ...
  return (
    <div style={mod.backdrop} onClick={onClose}><div style={mod.box} onClick={e => e.stopPropagation()}>
      <div style={mod.header}><span style={{ fontFamily: "'Bebas Neue'", fontSize: 24, letterSpacing: '0.05em' }}>BOOK CATERING</span><button onClick={onClose} style={{ fontSize: 20, color: '#8a8078' }}>✕</button></div>
    ...
    <>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', background: scrolled ? 'rgba(10,10,10,0.92)' : 'transparent', backdropFilter: scrolled ? 'blur(20px)' : 'none', borderBottom: scrolled ? '1px solid #2a2a2a' : '1px solid transparent', transition: 'all 0.4s ease' }}>
        ...
        <div className="nav-mobile" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={onCartClick} style={{ fontSize: 18, position: 'relative' }}>🌮{cartCount > 0 && <span style={{ position: 'absolute', top: -6, right: -8, background: '#D42B20', color: '#fff', fontSize: 9, fontWeight: 700, width: 16, height: 16, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{cartCount}</span>}</button>
          <button onClick={() => setMobileNav(!mobileNav)} style={{ fontSize: 22, color: '#F5F0E8' }}>{mobileNav ? '✕' : '☰'}</button>
        </div>
      </nav>
      {mobileNav && <div style={{ position: 'fixed', top: 64, left: 0, right: 0, bottom: 0, zIndex: 199, background: 'rgba(10,10,10,0.96)', backdropFilter: 'blur(20px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 32 }}>
        {['Menu', 'Catering', 'Locations'].map(l => <a key={l} href={`#${l.toLowerCase()}`} onClick={() => setMobileNav(false)} style={{ fontFamily: "'Bebas Neue'", fontSize: 36, letterSpacing: '0.1em', color: '#F5F0E8' }}>{l.toUpperCase()}</a>)}
...
const tag = { fontFamily: "'JetBrains Mono'", fontSize: 10, letterSpacing: '0.25em', color: '#D42B20', marginBottom: 12, textAlign: 'center' }
...
const hero = {
  ...
  h1: { fontFamily: "'Bebas Neue'", fontSize: 'clamp(60px, 14vw, 160px)', letterSpacing: '0.06em', lineHeight: 0.9, color: '#F5F0E8', display: 'inline-block' },
  sub: { fontFamily: "'DM Sans'", fontWeight: 300, fontSize: 'clamp(14px, 2vw, 18px)', lineHeight: 1.7, color: '#8a8078', margin: '24px 0 40px' },
...
const press = {
  wrap: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 28, padding: '16px 24px', background: '#0e0e0e', borderBottom: '1px solid #1e1e1e', flexWrap: 'wrap' },
  label: { fontFamily: "'JetBrains Mono'", fontSize: 9, letterSpacing: '0.2em', color: '#5a5450' },
...
const drw = {
  ...
  panel: { position: 'fixed', top: 0, right: 0, bottom: 0, width: 380, maxWidth: '100vw', zIndex: 299, background: '#0a0a0a', borderLeft: '1px solid #2a2a2a', display: 'flex', flexDirection: 'column', transition: 'transform 0.4s cubic-bezier(.16,1,.3,1)' },
  ...
  select: { width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid #2a2a2a', background: '#151515', color: '#F5F0E8', fontSize: 13, fontFamily: "'DM Sans'" },
  items: { flex: 1, overflowY: 'auto', padding: '0 20px' },
  item: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 0', borderBottom: '1px solid #2a2a2a' },
  qtyRow: { display: 'flex', alignItems: 'center', gap: 4 },
  qtyBtn: { width: 28, height: 28, borderRadius: '50%', border: '1px solid #2a2a2a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#F5F0E8', background: '#151515', cursor: 'pointer' },
  footer: { padding: 20, borderTop: '1px solid #2a2a2a' },
...
const mod = {
  ...
  box: { background: '#0a0a0a', border: '1px solid #2a2a2a', borderRadius: 16, width: '100%', maxWidth: 440, maxHeight: '90vh', overflowY: 'auto' },
  ...
  label: { fontSize: 12, fontWeight: 600, color: '#F5F0E8', letterSpacing: '0.05em' },
  input: { padding: '10px 14px', borderRadius: 8, border: '1px solid #2a2a2a', background: '#151515', color: '#F5F0E8', fontSize: 14, fontFamily: "'DM Sans'" },
}
