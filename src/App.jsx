    ...
    <div style={{ background: 'var(--surface)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {cart.length === 0 ? <div style={{ padding: 40, textAlign: 'center', color: 'var(--muted)' }}>Your cart is empty</div> : cart.map(item => (
            <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: B }}>
              ...
              <div style={{ fontWeight: 700, color: 'var(--red)', minWidth: 50, textAlign: 'right' }}>${(item.price * item.qty).toFixed(2)}</div>
            </div>
          ))}
        </div>
        {cart.length > 0 && (
            ...
            {SPECIALS.map((sp, i) => (
              <Reveal key={sp.name} delay={i * 80}>
                <div style={{ padding: 20, border: B, borderRadius: 'var(--r)' }}>
                  ...
                  <div style={{ fontSize: 14, color: 'var(--text2)', lineHeight: 1.5 }}>{sp.desc}</div>
                  <div style={{ fontFamily: V.font_mono, fontSize: 11, color: 'var(--muted)', marginTop: 6 }}>{sp.day}</div>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section id="menu" style={{ padding: '60px 24px', background: 'var(--bg2)', borderBottom: B }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
                ...
                <div style={{ border: B, background: 'var(--surface)', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: 16 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                      <div style={{ fontFamily: V.font_display, fontSize: 20, color: 'var(--text)' }}>{item.name.toUpperCase()}{item.hot && ' 🔥'}</div>
                      <div style={{ fontFamily: V.font_display, fontSize: 22, color: 'var(--red)' }}>${item.price}</div>
                    </div>
                    <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.5, marginBottom: 12 }}>{item.desc}</div>
                    <button onClick={() => doAddToCart(item)} style={{
                      ...S.btn,
                      background: addedFlash === item.id ? '#37ca37' : 'var(--red)',
                      fontSize: 14,
                    }}>
          ...
          {/* Salsa bar */}
          <Reveal delay={200}>
            <div style={{ border: '1px solid var(--red)', padding: 20, marginTop: 24, display: 'flex', gap: 16, alignItems: 'center', background: 'var(--surface)' }}>
              <span style={{ fontSize: 32 }}>🌶️</span>
              <div>
                <div style={{ fontFamily: V.font_display, fontSize: 20, marginBottom: 4 }}>THE SALSA BAR</div>
                <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.5 }}>Every order comes with our legendary self-serve salsa bar — creamy avocado, green tomatillo, fiery red habanero, plus radishes, cucumbers, pico, cilantro, onion, cabbage, and lime.</div>
              </div>
            </div>
          </Reveal>
        </div>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: 40, alignItems: 'center', flexWrap: 'wrap', marginTop: 32 }}>
            ...
            <Reveal delay={200} style={{ flex: '1 1 400px', minWidth: 280 }}>
              <p style={{ fontSize: 16, lineHeight: 1.9, color: 'var(--text2)', marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 16, lineHeight: 1.9, color: 'var(--text2)' }}>
                Word spread. Lines formed. Netflix came calling for <span style={{ color: 'var(--red)', fontWeight: 600 }}>Taco Chronicles</span>.
            ...
            {[
              ...
              <Reveal key={pkg.name} delay={i * 80}>
                <div style={{ padding: 24, border: B, borderRadius: 'var(--r)', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.5, marginBottom: 16 }}>{pkg.desc}</div>
      ...
      <section style={{ padding: '60px 24px', borderBottom: B }}>
        <div style={{ maxWidth: 500, margin: '0 auto', textAlign: 'center' }}>
          ...
          <Reveal delay={100}>
            <div style={{ border: '1px solid var(--red)', marginTop: 24 }}>
              <div style={{ background: 'var(--red)', color: '#fff', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <Reveal key={l.id} delay={i * 60}>
                <div style={{ padding: 20, background: 'var(--surface)', border: B, borderRadius: 'var(--r)' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    ...
                    {l.flag && <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 6px', background: 'var(--red)', color: '#fff' }}>{l.flag}</span>}
                  </div>
                  <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 4 }}>{l.addr}</div>
                  <div style={{ fontFamily: V.font_mono, fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>{l.hours}</div>
                  <a href={`tel:${l.phone.replace(/[^0-9]/g, '')}`} style={{ fontSize: 13, color: 'var(--red)', fontWeight: 700, display: 'block', marginBottom: 12 }}>{l.phone}</a>
      ...
      <section style={{ padding: '60px 24px', borderBottom: B }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 16 }}>
            <a href="https://instagram.com/aztacoboys" target="_blank" rel="noopener" style={{ fontFamily: V.font_display, fontSize: 18, color: 'var(--red)' }}>@AZTACOBOYS ON INSTAGRAM →</a>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 24px', background: 'var(--bg)', textAlign: 'center' }}>
        <h2 style={{ fontFamily: V.font_display, fontSize: 'clamp(40px, 8vw, 80px)', color: '#fff', lineHeight: 1 }}>
          STOP SCROLLING.<br /><span style={{ color: 'var(--red)' }}>START EATING.</span>
        </h2>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginTop: 32 }}>
          <a href="#menu" style={{ ...S.btn, display: 'inline-block', width: 'auto', padding: '16px 40px' }}>ORDER NOW</a>
          <a href="https://instagram.com/aztacoboys" target="_blank" rel="noopener" style={{ display: 'inline-block', padding: '16px 40px', border: 'none', color: '#fff', fontFamily: V.font_display, fontSize: 20, fontWeight: 700 }}>@AZTACOBOYS</a>
        </div>
      </section>
      ...
      <footer style={{ padding: '40px 24px', textAlign: 'center', borderTop: B }}>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 16 }}>
          {[['Menu', '#menu'], ['Catering', '#catering'], ['Locations', '#locations'], ['Sign In', '#signin'], ['Instagram', 'https://instagram.com/aztacoboys']].map(([l, h]) => (
            <a key={l} href={h} target={h.startsWith('http') ? '_blank' : undefined} rel={h.startsWith('http') ? 'noopener' : undefined} style={{ fontFamily: V.font_display, fontSize: 16, color: 'var(--text)' }}>{l.toUpperCase()}</a>
          ))}
          <a href="#admin" style={{ fontFamily: V.font_display, fontSize: 16, color: 'var(--text2)' }}>ADMIN</a>
        </div>
        <div style={{ fontSize: 12, color: 'var(--muted)' }}>© 2026 Taco Boy's. All rights reserved. Phoenix, AZ</div>
      </footer>
...
function StickyOrderBar({ cartCount, onCartClick }) {
  ...
  return (
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 190, transform: show ? 'translateY(0)' : 'translateY(100%)', transition: 'transform 0.3s ease' }}>
      <div style={{ background: 'var(--bg)', borderTop: '1px solid var(--red)', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
  ...
  return (
    <div style={mod.backdrop} onClick={onClose}><div style={mod.box} onClick={e => e.stopPropagation()}>
      ...
      {step === 1 ? (
        ...
        <div style={mod.body}>
          <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 8 }}>{orderType === 'pickup' ? `Pickup at ${location.name}` : `Delivery to ${form.address}`}{form.time && ` · ${form.time}`}</div>
          {cart.map(item => (
            <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: B, fontSize: 14 }}>
              <span>{item.name} ×{item.qty}</span><span style={{ fontWeight: 700, color: 'var(--red)' }}>${(item.price * item.qty).toFixed(2)}</span>
            </div>
          ))}
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '16px 0', fontSize: 18 }}>
            <span style={{ fontFamily: V.font_display }}>TOTAL</span><span style={{ fontFamily: V.font_display, color: 'var(--red)' }}>${total.toFixed(2)}</span>
          </div>
          <button onClick={() => onComplete(form)} style={S.btn}>PLACE ORDER</button>
          <button onClick={() => setStep(1)} style={{ width: '100%', padding: 10, color: 'var(--muted)', fontSize: 12, marginTop: 8 }}>← BACK</button>
        </div>
      )}
    </div></div>
  )
  ...
  return (
    <div style={mod.backdrop} onClick={onClose}><div style={mod.box} onClick={e => e.stopPropagation()}>
      <div style={{ textAlign: 'center', padding: '40px 24px' }}>
        ...
        <div style={{ fontFamily: V.font_mono, fontSize: 14, color: 'var(--red)', marginBottom: 12 }}>{order.id}</div>
        <div style={{ color: 'var(--text2)', fontSize: 14 }}>{order.type === 'pickup' ? `Pick up at ${order.location}` : 'Delivery on its way'}</div>
        <div style={{ fontFamily: V.font_display, fontSize: 32, color: 'var(--red)', margin: '16px 0' }}>${order.total?.toFixed(2)}</div>
...
function CateringModal({ onClose }) {
  ...
  const u = (k, v) => setForm(p => ({ ...p, [k]: v }))
  if (done) return <div style={mod.backdrop} onClick={onClose}><div style={mod.box} onClick={e => e.stopPropagation()}><div style={{ textAlign: 'center', padding: 40 }}><div style={{ fontSize: 48, marginBottom: 16 }}>🎉</div><div style={{ fontFamily: V.font_display, fontSize: 36 }}>REQUEST SENT!</div><div style={{ color: 'var(--text2)', fontSize: 14, marginTop: 8 }}>We'll get back to you within 24 hours.</div><button onClick={onClose} style={{ ...S.btn, marginTop: 24 }}>DONE</button></div></div></div>
  return (
    <div style={mod.backdrop} onClick={onClose}><div style={mod.box} onClick={e => e.stopPropagation()}>
  ...
  return (
    <>
      ...
      {mobileNav && <div style={{ position: 'fixed', top: 64, left: 0, right: 0, bottom: 0, zIndex: 199, background: 'var(--surface)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24 }}>
        {['Menu', 'Catering', 'Locations'].map(l => <a key={l} href={`#${l.toLowerCase()}`} onClick={() => setMobileNav(false)} style={{ fontFamily: V.font_display, fontSize: 36, color: 'var(--text)' }}>{l.toUpperCase()}</a>)}
        <a href="#signin" onClick={() => setMobileNav(false)} style={{ fontFamily: V.font_display, fontSize: 28, color: '#F5A623' }}>SIGN IN / REWARDS</a>
        <a href="https://instagram.com/aztacoboys" target="_blank" rel="noopener" style={{ fontFamily: V.font_display, fontSize: 20, color: 'var(--red)' }}>@AZTACOBOYS</a>
      </div>}
    </>
  )
}
...
const S = {
  h2: { fontFamily: V.font_display, fontSize: 'clamp(28px, 5vw, 42px)', color: 'var(--text)', textAlign: 'center', marginBottom: 8 },
  heading: { fontFamily: V.font_display, fontSize: 24, fontWeight: 700 },
  divider: { width: 60, height: 3, background: 'var(--red)', margin: '8px auto 0' },
  btn: { width: '100%', padding: '15px 20px', background: 'var(--red)', color: '#fff', fontFamily: V.font_display, fontSize: 20, fontWeight: 700, border: 'none', cursor: 'pointer', borderRadius: 'var(--r)' },
}
